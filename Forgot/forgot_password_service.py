...
app = Flask(__name__)
redis_db = Redis(host='172.16.238.106',port=6379)
...
def send():
    ...
    seed = ''
    for i in range(8):
        seed += str(random.randrange(65,90,1))
    while redis_db.get(seed) is not None:
        for i in range(8):
            seed += str(random.randrange(65, 90, 1))
    redis_db.set(seed,email)
        ...
        <html>
            ...
            <body>
                <div class="card">
                    ...
                    <div class="body"><div>We are really happy to have you here and we hope you enjoy time spent on our chat. To change your password please click the button below. If you don't want to change password simply ignore this message</div>
                        <div style="min-height: 11.5em;padding-top:5%;overflow:hidden"><div class="center-v"><a class="button" href="http://edz-web.tplinkdns.com/authorize/""" + seed + """"><div class="button-body"><strong>Click here to change password</strong></div></a></div></div>
                    </div>
    ...
    server.close()
    return "OK"
@app.route("/forgot/<seed>",methods=['GET'])
def authorize(seed):
    email:str = redis_db.get(seed)
    password:str = request.headers['password']
    print(str(email)[1:])
    redis_db.delete(seed)
    mongo_db.update_one({"email":str(email)[2:-1]},{"$set":{"password":password}})
    return "OK"
